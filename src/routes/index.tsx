import { createFileRoute, Link } from "@tanstack/react-router";
import heroImg from "../assets/hero.jpg";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Corporación Educacional San Joaquín | Concepción, Bío Bío" },
      {
        name: "description",
        content:
          "Centro educativo en Nonguén, Concepción. Calle Central 17, Valle Biobío. Teléfono (41) 249 9300. Abierto hasta las 18:30.",
      },
      {
        property: "og:title",
        content: "Corporación Educacional San Joaquín | Concepción",
      },
      {
        property: "og:description",
        content:
          "Centro educativo en Nonguén, Concepción. Calle Central 17, Valle Biobío.",
      },
      { property: "og:image", content: heroImg },
      { name: "twitter:image", content: heroImg },
    ],
  }),
  component: Index,
});

const HIGHLIGHTS = [
  {
    title: "Comunidad cercana",
    text: "Un colegio donde cada estudiante y su familia son conocidos por nombre, en el corazón de Nonguén.",
    icon: "heart",
  },
  {
    title: "Horario extendido",
    text: "Atendemos de lunes a viernes, abriendo temprano y cerrando a las 18:30 hrs para acompañar a las familias.",
    icon: "clock",
  },
  {
    title: "Ubicación accesible",
    text: "Calle Central 17, Valle Biobío, Nonguén. Fácil de llegar en locomoción y en auto desde Concepción.",
    icon: "pin",
  },
] as const;

function Icon({ name }: { name: "heart" | "clock" | "pin" }) {
  const common = { width: 24, height: 24, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 1.7, strokeLinecap: "round" as const, strokeLinejoin: "round" as const };
  if (name === "heart")
    return (
      <svg {...common}>
        <path d="M20.8 5.6a5 5 0 0 0-7.1 0L12 7.3l-1.7-1.7a5 5 0 1 0-7.1 7.1l8.8 8.8 8.8-8.8a5 5 0 0 0 0-7.1z" />
      </svg>
    );
  if (name === "clock")
    return (
      <svg {...common}>
        <circle cx="12" cy="12" r="9" />
        <path d="M12 7v5l3 2" />
      </svg>
    );
  return (
    <svg {...common}>
      <path d="M12 21s-7-6.2-7-11a7 7 0 1 1 14 0c0 4.8-7 11-7 11z" />
      <circle cx="12" cy="10" r="2.5" />
    </svg>
  );
}

function Stars({ value = 4 }: { value?: number }) {
  return (
    <div className="flex items-center gap-1" aria-label={`${value} de 5 estrellas`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} width="18" height="18" viewBox="0 0 24 24" fill={i < Math.round(value) ? "var(--color-accent)" : "none"} stroke="var(--color-accent)" strokeWidth="1.5">
          <path d="M12 2l3 6.5 7 .9-5 4.8 1.4 7L12 17.8 5.6 21.2 7 14.2 2 9.4l7-.9z" />
        </svg>
      ))}
    </div>
  );
}

function Index() {
  return (
    <div>
      {/* Hero */}
      <section className="relative">
        <div className="relative h-[78vh] min-h-[520px] w-full overflow-hidden">
          <img
            src={heroImg}
            alt="Fachada del colegio Corporación Educacional San Joaquín"
            width={1600}
            height={1000}
            className="absolute inset-0 h-full w-full object-cover"
          />
          <div className="absolute inset-0 hero-gradient" />
          <div className="relative mx-auto flex h-full max-w-6xl flex-col justify-end px-4 pb-16 sm:pb-20">
            <span className="mb-4 inline-flex w-fit items-center gap-2 rounded-full bg-accent px-3 py-1 text-xs font-medium text-accent-foreground">
              <span className="h-2 w-2 rounded-full bg-accent-foreground/80" /> Centro educativo · Nonguén
            </span>
            <h1 className="max-w-3xl text-balance text-4xl font-600 leading-tight text-primary-foreground sm:text-5xl md:text-6xl">
              Corporación Educacional San Joaquín
            </h1>
            <p className="mt-4 max-w-xl text-base text-primary-foreground/85 sm:text-lg">
              Un colegio del Valle Biobío en Concepción, donde la comunidad, la
              cercanía y el cariño forman a nuestros estudiantes.
            </p>
            <div className="mt-7 flex flex-wrap items-center gap-3">
              <Link
                to="/contacto"
                className="inline-flex items-center gap-2 rounded-md bg-accent px-5 py-3 text-sm font-600 text-accent-foreground transition-transform hover:scale-[1.02]"
              >
                Contáctanos
              </Link>
              <Link
                to="/ubicacion"
                className="inline-flex items-center gap-2 rounded-md border border-primary-foreground/40 bg-primary-foreground/10 px-5 py-3 text-sm font-600 text-primary-foreground backdrop-blur transition-colors hover:bg-primary-foreground/20"
              >
                Cómo llegar
              </Link>
            </div>
          </div>
        </div>
      </section>

      {/* Quick info bar */}
      <section className="border-b border-border bg-card">
        <div className="mx-auto grid max-w-6xl gap-px divide-y divide-border sm:grid-cols-3 sm:divide-x sm:divide-y-0">
          <div className="flex items-start gap-3 px-4 py-6">
            <span className="mt-1 text-accent"><Icon name="pin" /></span>
            <div>
              <p className="text-xs font-600 uppercase tracking-wide text-muted-foreground">Dirección</p>
              <p className="mt-1 text-sm text-foreground">Calle Central 17, Valle Biobío, Nonguén, Concepción</p>
            </div>
          </div>
          <div className="flex items-start gap-3 px-4 py-6">
            <span className="mt-1 text-accent"><Icon name="clock" /></span>
            <div>
              <p className="text-xs font-600 uppercase tracking-wide text-muted-foreground">Horario</p>
              <p className="mt-1 text-sm text-foreground">Abierto · Cierra 18:30</p>
            </div>
          </div>
          <div className="flex items-start gap-3 px-4 py-6">
            <span className="mt-1 text-accent">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
                <path d="M22 16.9v3a2 2 0 0 1-2.2 2 19.8 19.8 0 0 1-8.6-3.1 19.5 19.5 0 0 1-6-6A19.8 19.8 0 0 1 2.1 4.2 2 2 0 0 1 4.1 2h3a2 2 0 0 1 2 1.7c.1.9.4 1.8.7 2.6a2 2 0 0 1-.4 2.1L8.1 9.6a16 16 0 0 0 6 6l1.2-1.2a2 2 0 0 1 2.1-.4c.8.3 1.7.6 2.6.7a2 2 0 0 1 1.7 2z" />
              </svg>
            </span>
            <div>
              <p className="text-xs font-600 uppercase tracking-wide text-muted-foreground">Teléfono</p>
              <p className="mt-1 text-sm text-foreground">
                <a href="tel:+56412499300" className="hover:text-accent">(41) 249 9300</a>
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* Highlights */}
      <section className="mx-auto max-w-6xl px-4 py-16">
        <div className="max-w-2xl">
          <h2 className="text-3xl font-600 text-foreground sm:text-4xl">Un colegio con historia y comunidad</h2>
          <p className="mt-4 text-base text-muted-foreground">
            Por más de una década, las familias de Nonguén y el Valle Biobío han
            confiado en la Corporación Educacional San Joaquín para la formación
            de sus hijos. Conozca lo que nos hace especiales.
          </p>
        </div>
        <div className="mt-10 grid gap-6 sm:grid-cols-3">
          {HIGHLIGHTS.map((h) => (
            <div key={h.title} className="rounded-2xl border border-border bg-card p-6 transition-shadow hover:shadow-md">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-secondary text-primary">
                <Icon name={h.icon} />
              </div>
              <h3 className="mt-4 text-xl font-600 text-foreground">{h.title}</h3>
              <p className="mt-2 text-sm text-muted-foreground">{h.text}</p>
            </div>
          ))}
        </div>
      </section>

      {/* Rating summary */}
      <section className="bg-secondary/50">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-8 px-4 py-16 sm:flex-row sm:items-stretch">
          <div className="flex flex-col items-center justify-center rounded-2xl border border-border bg-card px-10 py-8 text-center">
            <span className="font-display text-6xl font-600 text-primary">4,0</span>
            <div className="mt-3"><Stars value={4} /></div>
            <p className="mt-2 text-sm text-muted-foreground">22 reseñas en Google</p>
          </div>
          <div className="flex-1">
            <h2 className="text-2xl font-600 text-foreground sm:text-3xl">Lo que dice nuestra comunidad</h2>
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <blockquote className="rounded-xl border border-border bg-card p-5">
                <p className="text-sm text-foreground">
                  “El mejor colegio en el que he estado, extraño el colegio y a los profesores.”
                </p>
                <footer className="mt-3 text-xs text-muted-foreground">— Ana Escalona</footer>
              </blockquote>
              <blockquote className="rounded-xl border border-border bg-card p-5">
                <p className="text-sm text-foreground">
                  “Me encanta este colegio, es muy bueno. Cuidan bien a nuestro colegio.”
                </p>
                <footer className="mt-3 text-xs text-muted-foreground">— Paulina Machuca</footer>
              </blockquote>
            </div>
            <Link
              to="/resenas"
              className="mt-6 inline-flex items-center gap-2 text-sm font-600 text-primary hover:text-accent"
            >
              Ver todas las reseñas
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M13 6l6 6-6 6" /></svg>
            </Link>
          </div>
        </div>
      </section>

      {/* CTA */}
      <section className="mx-auto max-w-6xl px-4 py-16">
        <div className="overflow-hidden rounded-3xl bg-primary px-6 py-12 text-center sm:px-12">
          <h2 className="text-3xl font-600 text-primary-foreground sm:text-4xl">¿Quieres conocer el colegio?</h2>
          <p className="mx-auto mt-3 max-w-xl text-base text-primary-foreground/85">
            Visítanos en Calle Central 17, Nonguén, o llámanos al (41) 249 9300.
            Estaremos felices de recibirte.
          </p>
          <div className="mt-7 flex flex-wrap justify-center gap-3">
            <Link
              to="/contacto"
              className="inline-flex items-center gap-2 rounded-md bg-accent px-6 py-3 text-sm font-600 text-accent-foreground"
            >
              Escríbenos
            </Link>
            <a
              href="https://www.google.com/maps/dir/?api=1&destination=Corporacion+Educacional+San+Joaquin+Concepcion"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 rounded-md border border-primary-foreground/40 px-6 py-3 text-sm font-600 text-primary-foreground hover:bg-primary-foreground/10"
            >
              Cómo llegar
            </a>
          </div>
        </div>
      </section>
    </div>
  );
}
